...
import {
  ...
  SkuList,
  SaveResult,
} from "@app/fps/services/sku-interface";
import {
  FinancialInstitutionService,
  FinancialInstitution,
} from "@app/_services/shared/finantial-institution.service";
...
export class SkuEditComponent implements OnInit {
  ...
  setConditionalValidators() {
    ...
    this.skuEditForm
      .get("has_interest_flag")
      .valueChanges.subscribe((has_interest_flag) => {
  ...
  isFieldValid(field: string) {
    return (
      !this.skuEditForm.get(field).valid && this.skuEditForm.get(field).touched
    );
  }
  ...
  onSave() {
    if (!this.skuEditForm.valid) {
      ...
      this.skuEditFormContainerRef.nativeElement.scrollIntoView({
        behavior: "smooth",
      });
      return;
    }
    this.SkuService.updateSku(
      {
        ...
        quarterly_payment: this.skuEditForm.get("quarterly_payment").value
          ? 1
          : 0,
        half_yearly_payment: this.skuEditForm.get("half_yearly_payment").value
          ? 1
          : 0,
        package_type_id: this.skuEditForm.get("package_type_id").value + "",
        interest_rate:
          (this.skuEditForm.get("interest_rate").value
            ? this.skuEditForm.get("interest_rate").value
            : 0) + "",
        currency_code: this.skuEditForm.get("currency_code").value + "",
        validity_start_at: this.datePipe.transform(
          this.skuEditForm.get("validity_start_at").value,
          "yyyy-MM-dd"
        ),
        validity_end_at: this.datePipe.transform(
          this.skuEditForm.get("validity_end_at").value,
          "yyyy-MM-dd"
        ),
        required_docs: this.skuEditForm.get("required_docs").value + "",
        required_tenure: this.skuEditForm.get("required_tenure").value + "",
        agreement_mandatory:
          this.skuEditForm.get("agreement_mandatory").value + "",
        ...
        min_payment_amount:
          this.skuEditForm.get("min_payment_amount").value + "",
      },
      this.skuID
    ).subscribe(
      (res) => {
  ...
  onSaveRateDetails() {
    ...
    for (let x = 0; x < rateDetails.length; x++) {
      if (
        rateDetails[x].details_tenure > 0 &&
        rateDetails[x].details_interest_rate > 0
      ) {
  ...
  storeRates(rate) {
    this.SkuService.saveRates(rate).subscribe(
      (res) => {
        if (res.id) {
  ...
  loadInstitutionsList() {
    if (this.countryCode.length > 0) {
      this.institutionService
        .getFinancialInstition(this.countryCode)
        .subscribe({
          next: (response) => {
            if (response.success) {
              this.zone.run(() => {
          ...
          error: (error) => {
            if (error.error.message != undefined) {
              ...
              this.dialogService.showErrorDialog(
                "Error retrieve institutions list"
              );
            }
          },
        });
  ...
  rateCheckSum(activeRateIDs, skuID) {
    this.SkuService.checksumRates({
      activeRateIDs: activeRateIDs,
      skuID: skuID,
    }).subscribe();
  }
  ...
  openAddRateDetailsModal(check, i) {
    ...
    this.currentRateStart =
      this.rateAddForm.value.addRates[i].validity_start_at;
    ...
    this.SkuService.loadRateDetails(this.currentRateNo).subscribe(
      (data) => {
        // Set Rate values:
      ...
      (err) => {
        alert(
          "Details for new rates can be added only after saving. please save and re-edit."
        );
      }
    );
  }
  ...
  storeRatesDetails(rate) {
    this.SkuService.saveRateDetails(rate).subscribe(
      (res) => {
        if (res.id) {
