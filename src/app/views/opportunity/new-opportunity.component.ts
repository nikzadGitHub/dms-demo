...
export class NewOpportunityComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.route.queryParams
      .subscribe(params => {        
        ...
        if(this.company_id_from_lead != '' && this.company_id_from_lead !== undefined){
    ...
    this.appService.getQuery('/opportunity/get-customer?customer_name='+keyword).subscribe((data) => {
      if(data['data'].length > 0){
        ...
        data['data'].forEach(value => {            
          this.companies.push({id:value.id, name: value.company_name, external_id: value.external_id, data_area_id: value.data_area_id});          
        });
  ...
  companySearchById(id){
    ...
    this.appService.getQuery('/opportunity/get-customer/'+id).subscribe((data) => {      
  ...
  createOpportunity(){
    ...
    this.appService.postQuery('/opportunity/new', {customer_id: this.company_id, topic: this.topic}).subscribe((data) => {			      
